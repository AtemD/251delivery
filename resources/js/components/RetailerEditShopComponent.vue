<template>
        <div v-if="$can('update shops')">
            ...
            <div class="modal fade" :id="'edit-shop'+shopToUpdate.id" style="display: none;" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h4 class="modal-title">Edit {{shopToUpdate.name}}</h4>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">×</span>
                            </button>
                        </div>
                        
                        <form role="form" @submit.prevent="updateShop()">
...
<script>
    export default {
        props: ['shop', 'shoptypes'],

        mounted() {
            console.log('Retailer update shop Component mounted.')
        }, 
        data() {
            return {
                form: new Form({
                    name : this.shop.name,
                    shop_type: this.shop.shop_type_id,
                    description: this.shop.description,
                    email: this.shop.email,
                    phone_number: this.shop.phone_number,
                    banner_image: this.shop.banner_image,
                    logo_image: this.shop.logo_image,
                    average_preparation_time: this.shop.average_preparation_time,
                    
                }),
                shopToUpdate: this.shop,
        ...
        methods: {
            updateShop(){
                ...
                this.form.put('/dashboard/retailer/shops/'+this.shopToUpdate.slug).then(()=>{

                    $('#edit-shop' + this.shopToUpdate.id).modal('hide');
