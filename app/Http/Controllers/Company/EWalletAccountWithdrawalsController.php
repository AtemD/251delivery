...
class EWalletAccountWithdrawalsController extends Controller
{
    ...
    public function update(Request $request, EWalletAccount $e_wallet_account)
    {
        ...
        $validatedData = $request->validate([
            'withdraw_amount' => 'required|regex:/^\d+(\.\d{1,2})?$/',
        ]);

        // Check if the account active state and status
        ...
        return back()->with('success', 'Account Withdrawal Successfully');
    }
}
